...
(function() {
    ...
    var MicrosoftGraphIntegration = {
        ...
        getEnvironmentConfig: function() {
            // Use the Microsoft configuration if available
            var config = window.MICROSOFT_CONFIG;
            var clientId = config ? config.clientId : null;
            
            if (clientId && config) {
                console.log('Microsoft Graph integration configured with Azure credentials.');
                // Update authority with specific tenant
                this.authority = config.authority;
                // Update scopes from config
            ...
            try {
                ...
                var msalConfig = {
                    auth: {
                        ...
                        redirectUri: config ? config.redirectUri : window.location.origin,
                        postLogoutRedirectUri: config ? config.redirectUri : window.location.origin
                    },
                    cache: {
        ...
        handleAuthSuccess: function(response) {
            ...
            try {
                ...
                this.getUserProfile().then(function(profile) {
                    ...
                    // Update UI state when connected
                    if (window.OutlookUIIntegration && window.OutlookUIIntegration.updateConnectionStatus) {
                        window.OutlookUIIntegration.updateConnectionStatus(true);
                    }
        ...
        signOut: function() {
            ...
            if (this.msalInstance) {
                return this.msalInstance.logoutPopup().then(function() {
                    ...
                    if (window.OutlookUIIntegration && window.OutlookUIIntegration.updateConnectionStatus) {
                        window.OutlookUIIntegration.updateConnectionStatus(false);
                    }
    ...
    // Wait for MSAL to be available before initializing
    var msalCheckCount = 0;
    var maxMsalChecks = 50; // Maximum 5 seconds wait
    ...
    function waitForMSAL() {
        ...
        } else if (msalCheckCount < maxMsalChecks) {
            msalCheckCount++;
            // Wait a bit more for MSAL to load
            setTimeout(waitForMSAL, 100);
        } else {
            console.warn('MSAL library failed to load after 5 seconds. Microsoft Graph integration will not be available.');
        }
    }
