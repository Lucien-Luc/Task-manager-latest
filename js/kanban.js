...
window.kanbanManager = {
    ...
    createTaskCard: function(task) {
        ...
        card.innerHTML = `
            <div class="task-category">
                ...
                ${task.isOutlookSync ? '<span class="outlook-event-badge">📅 Outlook</span>' : ''}
            </div>
        ...
        // Add special styling for Outlook-synced tasks
        if (task.isOutlookSync) {
            card.classList.add('outlook-synced');
        }
    ...
    handleDrop: async function(e) {
        ...
        if ((this.draggedTask.status === 'blocked' || this.draggedTask.status === 'paused') && 
            ...
            // Enhanced permission check for blocked tasks during grace period
            const gracePeriodStart = localStorage.getItem('gracePeriodStart');
            const gracePeriodActive = gracePeriodStart && (Date.now() - parseInt(gracePeriodStart)) < (5 * 60 * 1000);
            
            // Allow blocked task movement during grace period or if user has permission
            const canMove = gracePeriodActive || 
                           window.overdueManager.canUserMoveBlockedTask(this.draggedTask, currentUser, newStatus);
